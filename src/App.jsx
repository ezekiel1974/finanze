...
export default function App() {
  ...
  // Caricamento dati globali condivisi
  useEffect(() => {
    // Ascolta il saldo iniziale (salvato come documento globale)
    ...
    // Ascolta le transazioni condivise
    const q = query(
  ...
  const handleSaveInitialBalance = async () => {
    ...
    await setDoc(doc(db, 'settings', 'globalBalance'), { initialBalance: newBalance }, { merge: true });
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            {isEditingBalance ? (
              <div className="flex items-center gap-2 mt-2 relative z-10">
                ...
                />
                <button onClick={handleSaveInitialBalance} className="p-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                  <Check size={20} />
                </button>
              </div>
            ) : (
